...
class Screen(Ui):
    ...
    def __screen_var__(self):
        ...
        self.myScreenImgs = Queue()
        self.getScreenImg_list = []
        self.writeScreenImg_list = []
    ...
    def btn_screen_ok_clicked(self, *args):
        if self.screenRecordState:
            ...
            # self.screenRecordProcess.join()
            # self.screenRecordProcess.close()
            return
    ...
    def getScreen(self):
        ...
        self.label_top_right.setText('截屏完成, 保存在: {}'.format(filename), maxt=30, sec=200)
        # return
    ...
    def mousePressEvent_screen(self, a0: QtGui.QMouseEvent) -> None:
        if self.btn_screen_record.isVisible() and a0.button() in [Qt.RightButton, Qt.MidButton]:
            self.label_top_right.setText('')


    def mouseReleaseEvent_screen(self, a0):
        if self.btn_screen_record.isVisible() and not self.label_screen_show.isVisible():
            self.label_screen_show.setVisible(True)
...
num = 0
screen = QApplication.primaryScreen()
img = screen.grabWindow(WID).toImage()

videowriter = cv2.VideoWriter('./Temp/myVideo.wmv', cv2.VideoWriter_fourcc(*"mp4v"), 10, (img.width(), img.height()))
...
while True:
    ...
    ptr.setsize(img.byteCount())
    strData = ptr.asstring()
    arr = numpy.array(ptr).reshape(img.height(), img.width(),4)
    img2 = cv2.cvtColor(arr,cv2.COLOR_RGBA2BGR)
    img3 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)
